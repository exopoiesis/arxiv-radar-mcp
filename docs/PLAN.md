...
> **U7 RESOLVED NATIVELY (2026-06-25, RESHENIE-137):** PDF-only papers are now
> handled by arxiv-radar-mcp itself as tier 3 in the fetch cascade
> (HTML -> e-print -> PDF).  Install `arxiv-radar-mcp[pdf]` to activate.
> Parse mechanics live in `corpus_core.pdf` (corpus-core[pdf] extra);
> arxiv-radar calls `corpus_core.pdf.parse_pdf()` via `_fetch_pdf()` in
> `fulltext.py`.  Image refs are rewritten from MinerU's native "images/"
> to "<id>.media/" so the download zip resolves correctly.  VRAM freed
> via `_release_pdf_vram()` in `_do_fetch` after each batch.
> `validate_arxiv_ids` payload updated: `pdf_only` bucket renamed
> `pdf_fallback` + `pdf_parser_available` field added.
> U14 (lab-corpus `ingest_arxiv_pdf`) remains supported for combined-image path.
>
> **U14 RESOLVED (2026-05-13):** `ingest_arxiv_pdf(arxiv_id)`
> and `ingest_url(url, paper_id?)` download server-side
> via `corpus_core.fetch_url` (shared arXiv throttle).
