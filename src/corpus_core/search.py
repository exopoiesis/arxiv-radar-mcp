...
All functions take an iterable of Paper records (already loaded in memory)
and return a list of (paper, score) tuples sorted desc by score, top-k.
...
def search_text(papers: Iterable[Paper], query: str, k: int = 10,
...
def search_semantic(papers: dict[str, Paper], query_vec: np.ndarray,
...
def search_hybrid(text_results: list[tuple[Paper, float]],
                  semantic_results: list[tuple[Paper, float]],
                  k: int = 10, rrf_k: int = 60) -> list[tuple[Paper, float]]:
    ...
    by_id: dict[str, Paper] = {p.arxiv_id: p for p, _ in text_results}
...
def similar_to(papers: dict[str, Paper], arxiv_id: str,
               index: EmbeddingIndex, k: int = 10
               ) -> list[tuple[Paper, float]]:
